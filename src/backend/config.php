...
define('MAIL_USER', 'dindoumahonore@gmail.com');  
...
function jwtDecode(string $token): array
{
    $parts = explode('.', $token);
    if (count($parts) !== 3)
        throw new RuntimeException('Token malformé', 401);
    [$h, $p, $s] = $parts;
    ...
    if (!$payload)
        throw new RuntimeException('Payload invalide', 401);
...
function getBearerToken(): ?string
{
    ...
    // Lire les headers bruts depuis apache_request_headers()
...
function validerEntrees(array $data): void
{
    array_walk_recursive($data, function ($val) {
        if (is_string($val) && detecterSQL($val)) {
            ...
            // Alerte directe sans dépendre de LogService
            try {
                $pdo = getPDO();
                $pdo->prepare(
                    'INSERT INTO alertes_securite (type, titre, description, severite, ip, statut, created_at)
               VALUES ("sql_injection", "Injection SQL détectée",
             :desc, "haute", :ip, "active", NOW())'  
                )->execute([
                    ':desc' => 'Payload : ' . substr($val, 0, 100),
                    ':ip' => getIP(),
                ]);
            } catch (PDOException) {
            }
...
{
    if (is_array($input))
        return array_map('cleanXSS', $input);
    if (!is_string($input))
        return $input;
...
function readJSON(): array
{
    ...
    validerEntrees($data);  // ← détecter d'abord sur données brutes
    return cleanXSS($data);  // ← nettoyer ensuite
}
