...
function sendOTPRetrait(): void
{
    ...
    $user = $stmt->fetch();
    if (!$user) { jsonError('Utilisateur introuvable.', 404); return; }
    ...
    $pdo->prepare('UPDATE otps SET utilise = 1 WHERE user_id = ? AND utilise = 0')
        ->execute([$payload['sub']]);
...
function depot(): void
{
    ...
    if ($montant < 500)     { jsonError('Montant minimum : 500 FCFA.', 422); return; }
    if ($montant > 2000000) { jsonError('Montant maximum : 2 000 000 FCFA.', 422); return; }
    if (!$source)           { jsonError('Source requise.', 422); return; }
...
function envoi(): void
{
    ...
    if (!$dest)             { jsonError('Compte destinataire requis.', 422); return; }
    ...
    // Vérifier OTP
    $stmt = $pdo->prepare(
        'SELECT o.id FROM otps o
         JOIN utilisateurs u ON u.id = o.user_id
         WHERE u.id = ? AND o.code_hash = ? AND o.utilise = 0 AND o.expire_a > NOW()
         LIMIT 1'
    );
    $stmt->execute([$payload['sub'], hash('sha256', $otp)]);
    $otpRow = $stmt->fetch();
    if (!$otpRow) { jsonError('Code OTP invalide ou expiré.', 401); return; }
    ...
    if (!$compteDest) { jsonError('Compte destinataire introuvable.', 404); return; }
    ...
    try {
        ...
        if ($compteSource['solde'] < $total) {
            $pdo->rollBack(); jsonError('Solde insuffisant.', 402); return;
        }
        if ($compteSource['numero'] === $dest) {
            $pdo->rollBack(); jsonError("Impossible d'envoyer à son propre compte.", 422); return;
        }
...
function retrait(): void
{
    ...
    if ($montant < 500)     { jsonError('Montant minimum : 500 FCFA.', 422); return; }
    if (!$mode)             { jsonError('Mode de retrait requis.', 422); return; }
    if (strlen($pin) < 4)   { jsonError('PIN requis (4-6 chiffres).', 422); return; }
    if (strlen($otp) !== 6) { jsonError('Code OTP requis.', 422); return; }
...
function historique(): void
{
    ...
    if (!empty($_GET['type']))   { $where[] = 't.type = ?';          $params[] = cleanXSS($_GET['type']); }
    if (!empty($_GET['statut'])) { $where[] = 't.statut = ?';        $params[] = cleanXSS($_GET['statut']); }
    if (!empty($_GET['date']))   { $where[] = 'DATE(t.created_at)=?';$params[] = cleanXSS($_GET['date']); }
...
function getDetail(): void
{
    ...
    if (!$code) { jsonError('Code requis.', 422); return; }
    ...
    $tx = $stmt->fetch();
    if (!$tx) { jsonError('Transaction introuvable.', 404); return; }
    jsonReponse($tx);
}
